    ...
    packages = with pkgs; [
      ...
      (writeShellApplication {
        ...
        text = ''
          ...
          sudo tc filter add dev lo protocol ip parent 1:0 prio 1 u32 match ip dst 192.168.200.16/32 flowid 1:1
          sudo tc filter add dev lo protocol ip parent 1:0 prio 1 u32 match ip src 192.168.200.16/32 flowid 1:1
        '';
      })
      (writeShellApplication {
        name = "tc-reset";
