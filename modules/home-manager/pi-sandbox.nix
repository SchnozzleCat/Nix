    ...
    let
        ...
        text = ''
          ...
          ${optionalString cfg.forwardGpg ''
            ...
            if [ -S "$GPG_SOCKET" ] && [ -d "$HOME/.gnupg" ] && [ -d "$HOME/.password-store" ]; then
              ...
              ENV_INLINE+=("GNUPGHOME=/home/sandbox/.gnupg")
              ENV_INLINE+=("PASSWORD_STORE_DIR=/home/sandbox/.password-store")
            else
          ...
          # Port forwarding flags (from sandbox -p)
          declare -a PORT_FLAGS=()
          if [ -n "''${PI_SANDBOX_PORTS:-}" ]; then
            read -ra PORT_FLAGS <<< "$PI_SANDBOX_PORTS"
          fi

          # Copy home config files into tmpfs home on each run
          # shellcheck disable=SC2016
          INIT='cp -rL /etc/homeConfig/. "$HOME/" 2>/dev/null; '

          CONTAINER_CMD=("bash" "-c" "''${INIT}exec pi $*")
          if [ "''${PI_SANDBOX_SHELL:-}" = "1" ]; then
            if [ -n "''${PI_SANDBOX_CMD:-}" ]; then
              CONTAINER_CMD=("bash" "-c" "''${INIT}exec bash -c $(printf '%q' "$PI_SANDBOX_CMD")")
            else
          ...
          "''${RUN_PREFIX[@]}" \
            ...
            "$IMAGE" \
            env "''${ENV_INLINE[@]+"''${ENV_INLINE[@]}"}" "''${CONTAINER_CMD[@]}"
        '';
      };
      sandbox = pkgs.writeShellApplication {
        ...
        text = ''
          ...
          export PI_SANDBOX_SHELL=1
          export PI_SANDBOX_PORTS="''${PORTS[*]}"

          # Start socat forwarders for each -p port so localhost:<port> works on the host
          SOCAT_PIDS=()
          cleanup() {
          ...
          if [ $# -eq 0 ]; then
            ...
            PI_SANDBOX_CMD="$*" ${wrapper}/bin/pi
          fi
        '';
      };
    in {
      home.packages = [wrapper sandbox];
